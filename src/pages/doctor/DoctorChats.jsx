...
export default function DoctorChats() {
  ...
  useEffect(() => {
    ...
    s.on('chat:receive', (msg) => {
      if (selectedPatient && (msg.senderId === selectedPatient.userId || msg.receiverId === selectedPatient.userId)) {
  ...
  async function loadConversations() {
    try {
      setLoading(true);
      const result = await getDoctorConversations(token);
      if (result.success) {
  ...
  async function loadMessages(patientId) {
    try {
      setLoading(true);
      const result = await fetchChatHistory(patientId, token);
      if (result.success) {
  ...
  async function handleSend() {
    if (!messageText.trim() || !selectedPatient || sending) return;

    try {
      setSending(true);
      const result = await sendChatMessage({ to: selectedPatient.userId, message: messageText }, token);
      if (result.success) {
        const newMessage = {
          ...result.data,
        ...
        setMessageText('');
        socket?.emit('chat:send', { to: selectedPatient.userId, message: messageText });
          ...
          <div className="lg:col-span-1">
            <div className="rounded-2xl border border-slate-200 bg-white/80 shadow-lg overflow-hidden">
              <div className="border-b border-slate-100 px-4 py-3 bg-gradient-to-r from-indigo-500 via-sky-400 to-emerald-400">
                ...
                <p className="text-xs text-white/80 mt-1">{conversations.length} conversation{conversations.length !== 1 ? 's' : ''}</p>
              </div>
                  ...
                  conversations.map((conv) => (
                    <button
                      ...
                      className={`w-full text-left p-4 border-b border-slate-100 transition hover:bg-slate-50 ${
                        selectedPatient?.userId === conv.userId ? 'bg-indigo-50 border-indigo-200' : ''
                      }`}
                    >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white font-semibold">
                      {selectedPatient.profilePic ? (
                        <img src={`/Images/patients/${selectedPatient.profilePic}`} alt={selectedPatient.name} className="w-10 h-10 rounded-full object-cover" />
                      ) : (
                    ...
                    <div>
                      ...
                      <div className="text-xs text-white/80">{selectedPatient.email}</div>
                    </div>
                  </div>
                </div>
