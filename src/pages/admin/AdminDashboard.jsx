...
export default function AdminDashboard() {
  ...
  return (
    <AdminPageShell stats={{ users: users.length, patients: patientCount, doctors: doctorCount }}>
      <AdminPageHeader
        title="Admin Dashboard"
        subtitle="Review user profiles and remove accounts with proper reason tracking."
        actions={
          <button
            type="button"
            onClick={() => navigate('/admin/doctor-applications')}
            className="rounded-xl bg-[#5B3FA8] px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#5B3FA8]/25 transition hover:bg-[#4a3289] hover:shadow-xl active:scale-[0.98]"
          >
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <button
          ...
          className={`rounded-2xl bg-white p-4 text-left shadow-sm ring-1 transition hover:scale-[1.02] dark:bg-slate-900 ${
            userFilter === 'active'
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        <button
          ...
          className={`rounded-xl px-4 py-2 text-sm font-semibold transition active:scale-[0.98] ${
            roleFilter === 'all'
              ? 'bg-[#5B3FA8] text-white shadow-md'
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full rounded-xl border border-[#E8E0F5] bg-white px-4 py-3 text-sm outline-none transition focus:ring-2 focus:ring-[#5B3FA8] dark:border-slate-700 dark:bg-slate-900"
        />
      </div>

      {error && <p className="mb-4 text-red-600">{error}</p>}
      {loading ? (
        <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-[#E8E0F5] dark:bg-slate-900 dark:ring-slate-700">
          Loading users...
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          <section className="rounded-2xl border border-[#E8E0F5] bg-white p-4 shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="max-h-[34rem] space-y-3 overflow-y-auto pr-1">
              {visibleUsers.map((u) => (
                <div
                  key={u._id}
                  className="rounded-xl border border-[#E8E0F5] p-3 transition hover:border-[#5B3FA8]/40 hover:shadow-sm dark:border-slate-700"
                >
                  ...
                  <input
                    ...
                    className="mt-3 w-full rounded-lg border border-[#E8E0F5] px-2 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-800"
                  ...
                  <input
                    ...
                    className="mt-2 w-full rounded-lg border border-[#E8E0F5] px-2 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-800"
                    disabled={(u.accessStatus || 'active') === 'deleted'}
                  />
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={() => setSelectedUser(u)}
                      className="rounded-lg bg-[#5B3FA8] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#4a3289] active:scale-[0.98]"
                    >
                      View Profile
                    </button>
                    {(u.accessStatus || 'active') === 'deleted' ? (
                      <button
                        ...
                        className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-700 active:scale-[0.98]"
                      >
                        Restore
                      </button>
                    ) : (
                      <button
                        ...
                        className="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-rose-700 active:scale-[0.98]"
                      >
                        Remove
                      </button>
                    )}
          ...
          <section className="rounded-2xl border border-[#E8E0F5] bg-white p-4 shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-900">
            ...
            {selectedUser ? (
              <div className="space-y-2 text-sm">
                ...
                <p>
                  <span className="font-semibold">Specialty:</span> {selectedUser.specialty || '-'}
                </p>
                <p>
                  <span className="font-semibold">Location:</span> {selectedUser.location || '-'}
                </p>
                <p>
                  <span className="font-semibold">Bio:</span> {selectedUser.bio || '-'}
                </p>
                <p>
                  <span className="font-semibold">Access Status:</span> {selectedUser.accessStatus || 'active'}
                </p>
                <p>
                  <span className="font-semibold">Reason:</span> {selectedUser.accessReason || '-'}
                </p>
                <p>
      ...
      {popup.open && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/35 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl dark:bg-slate-900">
            <h3
              className={`text-lg font-bold ${popup.tone === 'error' ? 'text-rose-600' : 'text-emerald-600'}`}
            >
              {popup.title}
            </h3>
            ...
            <div className="mt-5 flex justify-end">
              <button
                ...
                className="rounded-lg bg-[#5B3FA8] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#4a3289] active:scale-[0.98]"
              >
                OK
              </button>
            </div>
